...
function initialize(){
  { // 難易度表を作成
    ...
    for(var A = 1 ; A <= 100 ; A++){
      for(var B = 1 ; B <= 100 ; B++){
        if(B / A > 4) continue; // 線分 B が線分 A に対して長すぎるパターンは除外
...
function func(){
        ...
        { // 2つの線分それぞれの長さを求める
          ...
          { // 線分 A の取りうる長さの範囲を求める
            ...
            lineSegmentA_PossibleRange = [
              Math.max(minLength, minLength*B_over_A),
              Math.min(lineSegmentA_MaxLength, lineSegmentB_MaxLength*A_over_B) 
              ];
          }
  ...
  { // canvas に描画
    ...
    
    {
      ...
      { // 線分 A を描画
        ctx.strokeStyle = 'rgba(107, 106, 216, 0.8)';
