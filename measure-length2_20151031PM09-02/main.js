...
// スコープを func の外に漏らすためにここで宣言
var ratioOfLineSegments;
var lineSegmentA_Angle;
var lineSegmentB_Angle;
  ...
  { // DOMツリーをいじる
    $('input[type="text"]#answer_field').keydown(function (e) {
      function answer(){
        ...
        var trueAnswer = ratioOfLineSegments[1];
        var userAnswer = parseInt( $('input[type="text"]#answer_field').val() );
        var isCorrect;
        
        $('span#warning_label').queue([]);
        $('span#warning_label').stop();
        $('span#warning_label').hide();
        
        { // エラーチェック
          var warningFlag = false;
          
          { // 解答が NaN だったら警告
            if( isNaN(userAnswer) ) warningFlag = true;
          }
          { // ユーザが線分 A と線分 B を逆に考えてしまっていたら警告
            if(ratioOfLineSegments[0] >= ratioOfLineSegments[1]){
              if(ratioOfLineSegments[0] < userAnswer) warningFlag = true; 
            }else{
              if(ratioOfLineSegments[0] >= userAnswer) warningFlag = true;
            }
          }
          ...
          if(warningFlag){
            $('span#warning_label').text('解答が異常です');
            $('span#warning_label').fadeIn('fast');
            
            return; // 再解答させる
          }
        }
        
        if(isCorrect = (userAnswer == trueAnswer)){ // 正誤判定
        ...
        { // ログを吐く
          $('#log').val(
            ...
            + '--------------------------------' + '\n'
            + 'date' + '\t'
            + 'ratioOfLineSegments[0]' + '\t'
            + 'ratioOfLineSegments[1]' + '\t'
            + 'lineSegmentA_Angle' + '\t'
            + 'lineSegmentB_Angle' + '\t'
            + 'lineSegmentA_Length' + '\t'
            + 'lineSegmentB_Length' + '\t'
...
function func(){
  ...
  lineSegmentA_Angle = 2.0 * Math.PI * Math.random();
  lineSegmentB_Angle = 2.0 * Math.PI * Math.random();
